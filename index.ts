let timers: number[] = [];
let intervals: Map<number, number> = new Map();
let timerElements: Map<number, Element> = new Map();

let startTimer = (timerId: number) => {
...
function countdown(duration: any, display: Element, timerId: number) {
...
function stopCountdown(timerId: number) {
    clearInterval(intervals.get(timerId));
}

let stopTimer = (timerId: number) => {
    stopCountdown(timerId);
}
...
let addTimer = () => {
    ...
    let nameInput = document.body.querySelector("#name") as HTMLInputElement;
